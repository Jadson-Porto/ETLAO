...
class ETL(AbstractETL):
    ...
    def load(self):
        try:
            ...
            if not cliente_df.empty:
                ...
                for _, row in cliente_df.iterrows():
                    cliente_obj = Cliente(
                        ...
                        Cpf=row.get('Cpf', '00000000000')
                    )
                    self.session.merge(cliente_obj)
                self.session.commit()
            else:
            ...
            # 5. Inserir Reserva - com validação e criação de Data ausente
            for _, row in self.transformed_data.get('reserva', pd.DataFrame()).iterrows():
                # Validar e garantir existência das datas de início e fim
                for data_val, nro_quarto_col in [
                    ...
                    if data_val and nro_quarto_col:
                        ...
                        if not existing_data:
                            ...
                            self.session.add(new_data)
                            self.session.flush()  # Para garantir o INSERT antes da reserva
